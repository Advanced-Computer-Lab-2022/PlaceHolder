...
export const updateRatingCourse = createAsyncThunk(
  'auth/updateRatingCourse',
  async (userData, thunkAPI) => {
    try {
      return await authService.updateRatingCourse(userData)
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const { reset12 } = authSlice.actions
